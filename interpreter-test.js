...
function interpret(data){
	...
	fs.readFileSync(data).toString().split('\n').forEach(function (line) {
		...
		if(bettingStarted && preflop == true && (line.includes(" calls ") || line.includes(" bets ") || line.includes(" raises ") || line.includes(" folds ") || line.includes(" checks "))){
			...
			//console.log("\nBEFORE");
			//console.log(usersContributions);
			fileOutput = getAction(line, currentUser, currentPotSize, usersContributions, gameId);
			//console.log("AFTER");
			//console.log(usersContributions);
		}
...
function getAction(lineVar, currentUserVar, potBlind, usersContributions, gameId){
	var fileOutput = '';
	...
	if(lineVar.includes(" raises ")){
		var raise = getPreflopRaise(lineVar, potBlind, gameId);
		...
		else if(betAmountInPots < 0.3 && betAmountInPots > 0){
			betAmountInPots = 0.2;
		}
		else if (betAmountInPots >= 0.3 && betAmountInPots < 0.6){
			betAmountInPots = 0.5;
		}
		else if (betAmountInPots >= 0.6 && betAmountInPots < 0.8){
			betAmountInPots = 0.7;
		}
		else if (betAmountInPots >= 0.8){
			betAmountInPots = 1;
		}
		if (betAmountInPots != 'a' && betAmountInPots != 0.2  && betAmountInPots != 0.5 && betAmountInPots != 0.7 && betAmountInPots != 1) {
...
function getPreflopRaise(lineVar, potBlind, gameId){
	var raiseAmount = parseInt(lineVar.split(" ")[4]);
	...
	else if(numberOfBigBlinds <= 0.3 && numberOfBigBlinds > 0){
		numberOfBigBlinds = 0.2;
	}
	else if(numberOfBigBlinds < 0.8 && numberOfBigBlinds > 0.3){
		numberOfBigBlinds = 0.5;
	}
	else if (numberOfBigBlinds >= 0.8 && numberOfBigBlinds < 1.2){
		numberOfBigBlinds = 1;
	}
	else if (numberOfBigBlinds >= 1.2 && numberOfBigBlinds < 1.4){
		numberOfBigBlinds = 1.3;
	}
	else if (numberOfBigBlinds >= 1.4){
		numberOfBigBlinds = 2;
	}
	if (numberOfBigBlinds != 'a' && numberOfBigBlinds != 0.2 && numberOfBigBlinds != 0.6 && numberOfBigBlinds != 0.5 && numberOfBigBlinds != 1.3 && numberOfBigBlinds != 1 && numberOfBigBlinds != 2) {
